	...
	function ajax_pagination() {
		...
		if ( !is_null($pager->_extra) ) {
			...
			foreach ( $pager->get($_POST['page']) as $key => $photo ) : 
				$html .= '<div class="flickr-thumb"><a href="<?php echo $phpFlickr->buildPhotoURL($photo, 'square') ?>"><img class="' . $photo['media'] . '" title="' . str_replace("\"", "\\\"", $photo['title']) . '" src="' . $phpFlickr->buildPhotoURL($photo, 'square') .'" alt="' .  str_replace("\"", "\\\"", $photo['title']) .'" /></a></div>';
			endforeach;
		}
